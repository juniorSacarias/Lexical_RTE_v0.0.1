...
const useOnClickListener = () => {
	...
	const updateToolbar = useCallback(() => {
		...
		const pushInEventTypesState = (selectionFormat, event) => {
			if (selectionFormat) { // If the selectionFormat is true.
		...
		if ($isRangeSelection(selection)) {
			...
			if (elementDOM !== null) { // If the elementDOM is not null.
				setSelectedElementKey(elementKey); // Sets the selectedElementKey to the elementKey.
				if ($isListNode(element)) { // If the element is a list node.
				...
				} else { // If the element is not a list node.
			...
			if ($isLinkNode(parent) || $isLinkNode(node)) { // If the parent is a link node or the node is a link node.
				if (!allSelectedEvents.includes(eventTypes.formatInsertLink)) // If the allSelectedEvents does not include the eventTypes.formatInsertLink.
			...
			} else { // If the parent is not a link node or the node is not a link node.
				if (allSelectedEvents.includes(eventTypes.formatInsertLink)) { // If the allSelectedEvents includes the eventTypes.formatInsertLink.
